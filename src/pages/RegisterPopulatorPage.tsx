  ...
  const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const response = await fetch(
        "https://fly-register-populator.fly.dev/register-populator/populate",
        {
          method: "POST",
      ...
      if (!response.ok) {
        ...
        throw new Error(
          `HTTP error ${response.status}: ${
            errorResult.message || "Failed to submit data"
          }`
        );
      }
      setSubmissionStatus({
        type: "success",
        message: `Google Sheets Register populated successfully!`,
      });
    } catch (err) {
      console.error("Error submitting form:", err);
      setSubmissionStatus({
        type: "error",
        message: `Error submitting form: Please contact Sa'ad for assistance`,
      });
    } finally {
      setIsLoading(false);
    }
