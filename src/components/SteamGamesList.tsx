		...
		<>
			...
			<div className="p-8">
				{loading ? (<>
					...
					<div className="flex justify-center items-center text-center h-32">The server was sleeping. Please wait while it wakes up.</div></>
					...
					<div className="grid gap-8 md:grid-cols-2 lg:grid-cols-3">
						{games.map((game, index) => (
							...
							>
								...
								/>
								<h3 className="text-xl font-bold skew-x-6" style={{ fontSize: 'clamp(0.75rem, 2.5vw, 1rem)' }}>
									{game.gameName}
								</h3>
								<p className="text-sm skew-x-6">Playtime: {Math.round(game.playTime / 60)} hrs</p>
							</div>
						))}
					</div>
				) : (
					<p>No games found. Note: The info retrieval on this page is rate limited.</p>
				)}
			</div>
		</>
	);
